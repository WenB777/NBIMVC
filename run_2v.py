    ...
    for missingrate in MR:
        ...
        for data_seed in range(1, args.test_time + 1):
            ...
            while epoch <= args.pre_epochs:
                NBIMVC.train_pre(config, logger, X_train_list[0], X_train_list[1], Y_list, mask, optimizer, epoch, device)
                epoch += 1

            while epoch <= args.pre_epochs + args.con_epochs:
                feature_all = NBIMVC.train_con(config, logger, X_train_list[0], X_train_list[1], Y_list, mask, optimizer, epoch, device, config['training']['lambda1'], 
                ...
                if epoch % 1 == 0:
                    with torch.no_grad():
                        scores_kmeans, scores_q,_,_ = NBIMVC.valid(config, logger, mask, X_train_list[0], X_train_list[1], Y_list, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pre_epochs', type=int, default=50, help='number of training epochs')
...
# nohup python run_2v.py > output_Scene-15_0.7.log 2>&1 &